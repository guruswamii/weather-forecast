...
function generateSuggestion(temp) {
    ...
    if (temp < 0) {
        ...
        imageURL = 'path/to/snow.jpg'; // Path to snow image
    } else if (temp < 10) {
...
function updateCurrentWeather(data, sunriseSunsetData) {
    ...
    if (sunriseSunsetData) {
        // Update sunrise, timezone, day length, and sunset information
    ...
    suggestionImageElement.style.width = '250px'; // Adjust the width as needed
    suggestionImageElement.style.height = '150px'; // Adjust the height as needed

    // Position the image below the suggestion text
    suggestionImageElement.style.display = 'block'; // Make the image a block element
    suggestionImageElement.style.margin = 'auto'; // Center the image horizontally
    suggestionImageElement.style.marginTop = '10px'; // Add margin at the top

    suggestionElement.appendChild(suggestionImageElement);
    fetchAirQuality(data.coord.lat, data.coord.lon)
    .then(result => {
        aqi = result; // Store the AQI globally
        updateAirQuality(aqi); // Update air quality information
    })
    .catch(error => {
        console.error('Error fetching air quality data:', error);
    });
...
function updateHourlyForecast(hourlyData) {
    ...
    hourlyForecastElement.innerHTML = ''; // Clear previous hourly forecast data
    ...
    if (Array.isArray(hourlyData)) {
        ...
        hourlyData.slice(0, 8).forEach((hourly, index) => { // Limiting to next 8 hours
            ...
            windCell.style.border = '1px solid black'; // Add border to table cells
            windCell.style.padding = '8px'; // Add padding to table cells
            row.appendChild(windCell);

            const humidityCell = document.createElement('td');
            humidityCell.textContent = `${hourly.main.humidity}%`;
            humidityCell.style.border = '1px solid black'; // Add border to table cells
            humidityCell.style.padding = '8px'; // Add padding to table cells
            row.appendChild(humidityCell);

            const visibilityCell = document.createElement('td');
            visibilityCell.textContent = `${hourly.visibility / 1000} km`;
            visibilityCell.style.border = '1px solid black'; // Add border to table cells
            visibilityCell.style.padding = '8px'; // Add padding to table cells
            row.appendChild(visibilityCell);

            // Apply different background colors to alternate rows for better readability
            if (index % 2 === 0) {
                row.style.backgroundColor = '#f2f2f2'; // Light gray color for even rows
            }
...
function updateForecast(forecastList) {
    ...
    dailyForecastElement.innerHTML = ''; // Clear previous forecast data
